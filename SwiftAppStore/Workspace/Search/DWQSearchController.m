...
-(void)initData{
    ...
    self.HotArr = [NSMutableArray arrayWithObjects:@"轮滑",@"高尔夫球",@"足球",@"网球",@"游泳",@"台球",@"护具",@"健身器材",@"篮球",@"乒乓球", nil];
...
{
    NSLog(@"点击了%@",KeyWord);
    for (NSString *stirng in self.historyArr) {
        if ([stirng isEqualToString:KeyWord]) {
            [_historyArr removeObject:stirng];
            break;
        }
    }
    ...
    CateSecondLevelViewController *cont = [[CateSecondLevelViewController alloc]init];
    cont.keyString = KeyWord;
